...
import {
  ...
  MapPin, Heart, Pill, ArrowLeft, KeyRound, ShieldCheck
} from 'lucide-react'
...
function ForgotPassword({ onBack }) {
  ...
  const sendCode = async (e) => {
    ...
    try {
      ...
      // Use the actual email returned by the server for subsequent steps
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <React.Fragment key={n}>
              ...
              {i < 2 && <div className={`flex-1 h-0.5 max-w-[40px] rounded ${step > n ? 'bg-[#14C8A8]' : 'bg-gray-200'}`} />}
            </React.Fragment>
          )
        })}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.form key="s1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
            onSubmit={sendCode} className="space-y-5">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2">Email address or Username</label>
              <div className="relative">
                ...
                <input type="text" value={email} onChange={e => { setEmail(e.target.value); setError('') }}
                  ...
                  className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0F6FFF]/30 focus:border-[#0F6FFF] bg-gray-50" />
              </div>
            </div>
